...
for /f "tokens=* delims= " %%a in ('dir /s/b/a-d "*.7z"') do (
SET /A VARNUM=16
SET FILN=
SET FILCNT=
SET FILNORM
...
set ZIPA=%%~nxa
set ZIPN=%%~dpnxa
"%SEVENZIP%" l -slt "%%~a">"%GBC%\tmp.ini"
CALL :ALLPROC
CALL :ZIPMOVE
)
del /q "%GBC%\7zextr.ini"
del /q "%GBC%\tmp.ini"
exit /b


:ZIPMOVE
for /f "tokens=1,2 delims=:" %%a in ('type "%GBC%\7zextr.ini"') do (
set ARCN=%%~a
set ARCF=%%~nxa
set ARCE=%%~xa
...
:GETME
if "%FILNORM%" NEQ "%FILCNT%" set /A VARNUM+=16
