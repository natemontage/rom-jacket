...
for /f "tokens=* delims= " %%a in ('dir /b /a-d "%GBG%\%GAM%\%CONS%\%X68K%\%CSTM%"') do (
...
if "%X68KHDT%"=="1" "%GBC%\inifile.exe" "XM6.ini" [Resume] Path=%GBG%\%GAM%\%CONS%\%X68K%\%CSTM%
if "%X68KHDT%"=="1" "%GBC%\inifile.exe" "XM6.ini" [SASI] File0=%X68KDSK%
"%GBC%\fart.exe" "*.bat" [DSK1] "%X68KDSK%"
if "%X68KDSK2%"=="" "%GBC%\fart.exe" "*.bat" --remove [DSK2]
if "%X68KDSK2%" NEQ "" "%GBC%\fart.exe" "*.bat" [DSK2] "%X68KDSK2%"
if "%X68KDSK3%" NEQ "" "%GBC%\fart.exe" "*.bat" [DSK3]
if "%X68KDSK4%" NEQ "" "%GBC%\fart.exe" "*.bat" [DSK4] "%X68KDSK2%"
popd
goto :x68kend
